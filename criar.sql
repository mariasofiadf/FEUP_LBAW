...
CREATE TABLE users(
    ...
    profile_image IMAGE, -- check type (VARBINARY())
...
CREATE TABLE auction(
    ...
    seller_id INTEGER REFERENCES user(user_id) NOT NULL
);
...
CREATE TABLE bid(
    ...
    bidder_id INTEGER REFERENCES user(user_id) NOT NULL
)
...
CREATE TABLE rating(
    id_rated INTEGER REFERENCES user(user_id) NOT NULL,
    id_rates INTEGER REFERENCES user(user_id) NOT NULL,
    rate_value INTEGER NOT NULL CHECK (rate_value > 0 && rate_value < 5),--change name, < 5 ??
    rate_date DATE NOT NULL,    --change name
    PRIMARY KEY(id_rated, id_rates)
)

CREATE TABLE user_follow(
    id_folllowed INTEGER REFERENCES user(user_id) NOT NULL,
    id_folllower INTEGER REFERENCES user(user_id) NOT NULL CHECK (id_folllowed != id_folllower), 
    rate_value INTEGER NOT NULL CHECK (rate_value > 0 && rate_value < 5),--change name, < 5 ??
    rate_date DATE NOT NULL,    --change name
...
CREATE TABLE user_notification(
    ...
    notif_category user_notification NOT NULL  --change name
)
...
CREATE TABLE auction_notification(
    ...
    anotif_category user_notification NOT NULL  --change name
)
